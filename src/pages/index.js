...
const IndexPage = () => {
  ...
  const fetchAllProducts = async () => {
    try {
      ...
      setAllProductList(productRes.data);
      const popularProductRes = await getProductList({ sort: 'total_sold', direction: 'desc' });
      setAllPopularProduct(popularProductRes.data);
      const newProductRes = await getProductList({ sort: "date_last_imported", direction: 'desc' });
      setAllNewProduct(newProductRes.data);
      setLoading(false);
    } catch (e) {
      setLoading(false);
      console.log(e)
